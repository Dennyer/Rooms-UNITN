...
def query_handler(update: Update, context: CallbackContext):
    ...
    if query_split[0] == "err":
        ...
        if query_split[1] == "selroom":
            update.callback_query.message.edit_text(DT['selectOptions'],
                                                    reply_markup=KEYBOARD.date_room_hedquarter_keyboard(query_split[2],
                                                                                                        query_split[3]))
        elif query_split[1] == "selroomtoday":
...
def send_rooms_day(update: Update, context: CallbackContext, head, room, date="none"):
    ...
    update.callback_query.message.edit_text(DT['viewResult'] + room + ":" + text,
                                            reply_markup=KEYBOARD.back_keyboard("room;selroom;" + head + ";" + room))
